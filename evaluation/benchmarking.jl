...
function isnearwrap(
    treea, treeb, nodea, nodeb; α=ButterflyFactorizations.tree_parameters(treea).α
)
...
function fit_scaling_factor(
    ...
    # Filter out NaNs (in case ACA was skipped)
...
function run_benchmarks(
    ...
    for h in h_values
        ...
        # Build Trees
        if treekind == :KMeansTree || checkfarfieldaccuracy
        ...
        if checkfarfieldaccuracy
            ...
            # Use identical admissibility (isnearwrap) to guarantee identical far-field blocks
            refmat = HMatrix(
                op,
                X,
                X,
    ...
    # --- FINAL RECOMPUTATION OF FITTED REFERENCE CURVES ---
    ...
    # Add ACA to plots only if we measured it
    if acacomparison
        push!(
            time_traces,
            scatter(;
...
p_time, p_mem, p_mv, p_err = run_benchmarks(
    ...
    acacomparison=false,          # Set to true to run standard ACA
