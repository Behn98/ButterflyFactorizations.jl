...
function run_benchmarks(
    ...
    # 🚀 NEW: Hard plot histories to prevent memory garbage collection on crash
    ...
    try
        ...
        for h in h_values
            ...
            push!(p_time_history, p_time) # 🚀 Pushed to history array
            ...
            push!(p_mem_history, p_mem) # 🚀 Pushed to history array
            ...
            push!(p_mv_history, p_mv) # 🚀 Pushed to history array
            ...
            push!(p_err_history, p_err) # 🚀 Pushed to history array
            ...
            push!(p_rank_history, p_rank_vs_k) # 🚀 Pushed to history array
    ...
    # 🚀 Safely retrieve the last generated plot from the arrays, or return empty plots if it failed on iteration 1
...
p_time, p_mem, p_mv, p_err, p_rank_vs_k, p_level_ranks_all = run_benchmarks(
    ...
    highfscaling=true,         # false: Locks wavenumber k to the finest mesh (isolates O(N log^2 N) scaling). true: Scales k with h (tracks physical radar cross-section growth).
    ...
    adaptive=true,             # true: Dynamically estimates rank bounds during compression. false: Uses fixed rank limits. (always set to true if rankestimator_type=:Butterfly)
